...
- **Key Features**:
  ...
  - Uses custom `bifurcation_1d_analyzer` class from `supplementary` folder. The latest version and examples for this class may be found in [Bifurcation 1D Analysis - MATLAB Central File Exchange](https://www.mathworks.com/matlabcentral/fileexchange/179659-bifurcation-1d-analysis).
...
- All files analyze the **Sang chaotic system**: `dx/dt = -y`, `dy/dt = x + cy + az`, `dz/dt = -μz + b·cos(ωy)`
- Use **Heun's method** (2nd-order Runge-Kutta) for numerical integration
- Focus on **hardware implementation** aspects
- Include **bifurcation analysis** and **chaos characterization**
- Support both **floating-point** and **fixed-point** implementations
